...
```{r}
rm(list=ls())
setwd("/Users/banj/Downloads")
rdata = read.csv("/Users/banj/Downloads/organics.csv")
...
train.df.nn.s = nn.data[ti,selected.var]
valid.df.nn.s = nn.data[-ti,selected.var]
...
# Getting the formula
n = names(train.df.nn)
ns= names(train.df.nn.s)
f = as.formula(paste("TargetBuy~", paste(n[!n %in% "TargetBuy"], collapse = "+")))
fs = as.formula(paste("TargetBuy~", paste(ns[!ns %in% "TargetBuy"], collapse = "+")))
organic.nn1 = neuralnet(f, data = train.df.nn, linear.output = F, hidden = 3,threshold = 0.1)

 organic.nn2 <- neuralnet(fs, data = train.df.nn.s, linear.output = F, hidden = 3,threshold = 0.1)
# organic.nn3 <- neuralnet(f, data = train.df.nn, linear.output = F, hidden = c(5,5),threshold = 0.1)
prediction(organic.nn1)
plot(organic.nn1)
organic.nn.pred1 <- compute(organic.nn1,valid.df.nn[,-6] )
organic.nn.pred2 <- compute(organic.nn2,valid.df.nn.s[,-3])

RMSE(ifelse(organic.nn.pred1$net.result >0.6,1,0), valid.df.nn$TargetBuy)
accuracy(unlist(organic.nn.pred1),valid.df.nn$TargetBuy)

library(caret)
result=ifelse(organic.nn.pred1$net.result >0.5,1,0)
confusionMatrix(table(result, valid.df.nn$TargetBuy))

result2=ifelse(organic.nn.pred2$net.result >0.5,1,0)
confusionMatrix(table(result2, valid.df.nn.s$TargetBuy))
...
=======
=======
>>>>>>> 7d1afc28d94c470b4b36b263a9c06f9eb7c6e58a
...
library(caret)
<<<<<<< HEAD
>>>>>>> 4b1f563c50010fd6c932f73f19a501631a1480c7
=======
result=ifelse(organic.nn.pred1$net.result > 0.5,1,0)
confusionMatrix(table(result, valid.df.nn$TargetBuy))
...
accT = c()
for(cutoff in seq(0, 1, 0.01)){
    cm = confusionMatrix(ifelse(organic.nn.pred1$net.result > cutoff, 1, 0), valid.df.nn$TargetBuy)
    accT = c(accT, cm$overall[1])
}
